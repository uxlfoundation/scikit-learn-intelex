...
def isolated_trace():
    p = subprocess.Popen(
        [sys.executable, "-m", "sklearnex.tests.test_common"],
...
if __name__ == "__main__":
        ...
        for path in sys.path:
            ...
            try:
                ...
                # add path to blacklist if not a sub path of anything in the whitelist
                    ...
                    [_commonpath([i, fpath]) != i for i in _TRACE_ALLOW_DICT.values()]
                ):
                    blacklist += [fpath]
    ...
    while info := input():  # no entry will terminate the loop
        sklearnex_trace(*(info.split("|")))
