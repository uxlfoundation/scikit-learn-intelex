...
- ``onedal.tests.utils._dataframes_support.pd_to_torch_working``: a boolean variable telling whether |sklearn|'s ``move_to`` can convert a Pandas object to a PyTorch tensor. It is ``False`` for versions affected by `scikit-learn#34046 <https://github.com/scikit-learn/scikit-learn/issues/34046>`__, where such a conversion raises an error instead of falling back to a host round-trip. Tests that pass a Pandas ``y`` or sample weight alongside a PyTorch ``X`` should skip on ``not pd_to_torch_working`` rather than skipping for PyTorch unconditionally, so that they start running again once the |sklearn| fix is released.
