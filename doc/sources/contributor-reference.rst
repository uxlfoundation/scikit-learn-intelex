...
- ``onedal.tests.utils._dataframes_support.mixed_device_params``: a generator of ``pytest.param`` entries for tests that pass inputs living on *different devices* to the same estimator, to be used with ``@pytest.mark.parametrize`` alongside the ``with_array_api`` fixture. It yields ``(X_xp, X_device, y_xp, y_device)`` tuples, optionally extended with a sample weight (``include_weight=True``). Since array API dispatching resolves a single namespace across all inputs, combinations that mix namespaces (e.g. PyTorch ``X`` with DPNP ``y``) are never emitted; only devices vary within one namespace. Pass ``include_host_df_y=True`` to also cover a namespace-neutral host data frame ``y``/weight, one per library in ``host_df_modules``, and ``x_devices`` to restrict ``X`` for estimators supporting only some devices (e.g. ``x_devices=("cpu",)``).
- ``onedal.tests.utils._dataframes_support.host_df_modules``: the tuple of host data frame libraries usable as a namespace-neutral ``y``/sample weight alongside any ``X`` — Pandas, plus Polars when it is installed. Tests fed by ``mixed_device_params(include_host_df_y=True)`` should branch on ``y_xp in host_df_modules`` rather than comparing against ``pd``, and construct data through the yielded module (``y_xp.Series(...)``) so that both libraries are exercised.
