...
- **Automatic array API dispatch for on-device inputs**: an ``autouse`` fixture (``_array_api_dispatch_for_device_frameworks`` in ``sklearnex/conftest.py``) inspects the ``dataframe`` parameter of each test and, when it is ``"dpnp"`` or ``"array_api"``, runs the test body inside ``with config_context(array_api_dispatch=True)``. This is applied automatically to every test parametrized with ``get_dataframes_and_queues`` — no per-test opt-in is needed. The rationale: non-NumPy array API inputs (e.g. dpnp) are only kept on device under ``array_api_dispatch``; without it they are silently transferred to host and computed as NumPy, which leaves the on-device code paths untested and can hide device-specific correctness bugs. Two consequences to keep in mind when writing tests:

  - Under dispatch, **fitted attributes and method outputs stay in the input's namespace** (a dpnp input yields dpnp results). Comparisons against a NumPy expected value must therefore convert the result first (see ``assert_allclose_numpy`` / ``_as_numpy`` below) — ``numpy.testing.assert_allclose`` cannot consume a dpnp array on either side.
  - Tests marked ``@pytest.mark.allow_sklearn_fallback`` are **left untouched** by this fixture (dispatch is not forced), since those tests intentionally exercise the host/fallback path. Use this mark for any dpnp test whose configuration is expected to fall back to |sklearn| rather than run on device. Note that such a test makes no statement about the |sklearnex| device code paths: |sklearn| supports ``array_api_dispatch`` for a growing set of estimators, so a fallback test given array API inputs may well succeed entirely within |sklearn| without ever entering ``sklearnex._device_offload``. Treat the mark as an assertion about fallback behavior only, and cover the on-device path with a separate non-fallback test.

- ``@pytest.mark.allow_sklearn_fallback``: will avoid having tests fail when they end up calling procedures from |sklearn| instead of from the |onedal|. This can be helpful for example when testing that some corner case falls back correctly when it should. It also disables the automatic array API dispatch described above.
...
- ``onedal.tests.utils._dataframes_support.assert_allclose_numpy``: a drop-in replacement for ``numpy.testing.assert_allclose`` that routes both operands through ``_as_numpy`` first, so either side may be a dpnp or other array API result. All remaining arguments (``rtol``, ``atol``, ``err_msg``, ...) are forwarded unchanged. Prefer it over calling ``_as_numpy`` at each comparison site.
- ``onedal.tests.utils._dataframes_support._assert_in_namespace``: asserts that an object belongs to the array namespace implied by the test's ``dataframe`` parameter (e.g. a ``"dpnp"`` test must produce a dpnp result), ignoring scalars. Use it once per test on a result-side value to verify that outputs stay on device (dpnp-in → dpnp-out) rather than being silently host-transferred. It is deliberately separate from ``assert_allclose_numpy`` so that the namespace check is an explicit, intentional assertion rather than a side effect of every value comparison. PyTorch is checked by tensor type instead of ``__array_namespace__``, which ``torch.Tensor`` does not implement since |sklearn| reaches PyTorch through ``array_api_compat``; an optional ``device`` argument (a SYCL queue, another array, or a ``torch.device``) additionally asserts the result landed on the expected device.
- ``onedal.tests.utils._dataframes_support.skip_array_api_strict_readonly``: skips a test when the ``dataframe`` parameter is ``"array_api"`` and NumPy is older than 2.2.5. Needed only for estimators that feed fitted attributes back through ``to_table`` (e.g. ``PCA``/``IncrementalPCA`` ``components_``, ``DummyRegressor`` ``constant_``), since older NumPy hands those arrays back read-only and the DLPack export rejects them.
...
- ``onedal.tests.utils._dataframes_support.mixed_device_params``: a generator of ``pytest.param`` entries for tests that pass inputs living on *different devices* to the same estimator, to be used with ``@pytest.mark.parametrize`` alongside the ``with_array_api`` fixture. It yields ``(X_xp, X_device, y_xp, y_device)`` tuples, optionally extended with a sample weight (``include_weight=True``). Everything follows ``X`` — the estimator moves ``y`` and sample weights to ``X``'s namespace and device — so only devices vary within one namespace here; cross-namespace ``X``/``y`` (e.g. PyTorch ``X`` with DPNP ``y``) is supported from |sklearn| 1.9 on, but requires both frameworks installed and is covered by the ``test_*_mixed_array_namespaces`` tests instead. Pass ``include_host_df_y=True`` to also cover a namespace-neutral host data frame ``y``/weight, one per library in ``host_df_modules``, and ``x_devices`` to restrict ``X`` for estimators supporting only some devices (e.g. ``x_devices=("cpu",)``).
- ``onedal.tests.utils._dataframes_support.host_df_modules``: the tuple of host data frame libraries usable as a namespace-neutral ``y``/sample weight alongside any ``X`` — Pandas and Polars, both mandatory test dependencies. Tests fed by ``mixed_device_params(include_host_df_y=True)`` should branch on ``y_xp in host_df_modules`` rather than comparing against ``pd``, and construct data through the yielded module (``y_xp.Series(...)``) so that both libraries are exercised.
- ``onedal.tests.utils._dataframes_support.host_df_to_torch_working``: a boolean variable telling whether |sklearn|'s ``move_to`` can convert a host data frame object (Pandas, Polars) to a PyTorch tensor. It is ``False`` for versions affected by `scikit-learn#34046 <https://github.com/scikit-learn/scikit-learn/issues/34046>`__, in which the host round-trip fallback for inputs lacking ``__dlpack__`` is skipped because PyTorch signals that case with an exception type that ``move_to`` does not catch. Other namespaces (e.g. ``array_api_strict``) convert such inputs without issue, and any host data frame is affected rather than just Pandas, so the Pandas probe behind this variable is representative of Polars too. Tests that pass a host data frame ``y`` or sample weight alongside a PyTorch ``X`` should skip on ``not host_df_to_torch_working`` rather than skipping for PyTorch unconditionally, so that they start running again once the |sklearn| fix is released.
