...
with current available projects described in the latest version of the documnetation.
...
known as BasicStatistics and IncrementalBasicStatistics. They generates relevant values like 'min', 'max', 'mean' 
and 'variance' with special focus on multithreaded performance. It is also MPI-enabled working on SBCs to multi-node 
clusters, and can prove very useful for important big data pre-processsing steps which may be otherwise unwieldly. 
...
the scikit-learn-intelex impelementation provides better performance. This project will focus on the public API and including the benchmarking 
