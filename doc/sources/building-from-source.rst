...
- ``--free-threading``: selects the free-threaded build configuration - the free-threaded code generation mode for the ``daal4py`` module, and the ``find_package(Python)`` CMake path for the |onedal| backends, which requires ``pybind11>=2.13``. This defaults to whether the interpreter running the build is itself free-threaded, so it is only needed to build the free-threaded configuration from a GIL-enabled interpreter. Note that whether the resulting extension modules use the free-threaded ABI is determined by the Python headers they are compiled against, not by this argument.
