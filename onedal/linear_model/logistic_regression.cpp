...
ONEDAL_PY_INIT_MODULE(logistic_regression) {
    ...
    // logistic_regression SPMD is exported by oneDAL only for the data-parallel
    // policy, so these must stay dpc-only to link.
    ONEDAL_PY_INSTANTIATE(init_train_ops, sub, policy_spmd_dpc, task_list);
    ONEDAL_PY_INSTANTIATE(init_infer_ops, sub, policy_spmd_dpc, task_list);
